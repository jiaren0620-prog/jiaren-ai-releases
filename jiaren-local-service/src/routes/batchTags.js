...
const { generateCurrentChat, publicProvider, resolveCurrentChatProvider } = require('../services/currentApi');
...
function buildBatchTagMessages(input = {}) {
  ...
  const url = String(item.url || item.imageUrl || item.videoUrl || item.localPath || item.dataUrl || item.source || item.sourcePath || '').trim();
...
function resolveBatchTagModel(provider, input = {}, kind = 'image') {
  let requested = cleanText(input.model || input.providerModel, 240);
  if (provider?.id !== 'zhenzhen' && requested === DEFAULT_ZHENZHEN_MODEL) {
    requested = '';
  }
...
function devSourceSearchRoots(projectRoot) {
  const out = [];
  const seen = new Set();
  for (const root of splitDevSourceRoots(process.env.JIAREN_BATCH_TAG_SOURCE_ROOTS)) {
...
function buildJsonPayload({
  ...
  return {
    ...
    sourceUrl: item.url || item.imageUrl || item.videoUrl || item.localPath || item.dataUrl || item.source || item.sourcePath || '',
...
router.post('/tag', async (req, res) => {
  ...
  try {
    ...
    if (String(item.kind || body.mediaKind || '').toLowerCase() === 'video') {
      return res.status(400).json({ success: false, code: 'video_module_removed', error: 'AI 视频模块已移除；批量打标当前只处理图像素材。' });
    }
    const resolved = resolveCurrentChatProvider(body.model || body.providerModel);
    if (!resolved.ok) {
      return res.status(400).json({
        success: false,
        code: resolved.code,
        error: resolved.error,
        data: resolved.provider ? { provider: publicProvider(resolved.provider) } : undefined,
      });
    }
    ...
    const result = await generateCurrentChat(resolved.provider, {
      model,
      messages,
    ...
    if (!result.ok) {
      return res.status(502).json({
        ...
        data: { provider: publicProvider(resolved.provider), raw: result.raw },
      });
    }
