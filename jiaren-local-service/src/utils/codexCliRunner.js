...
const CODEX_DISABLED_MESSAGE = 'Codex CLI 不可用：请先安装并登录 Codex CLI，或在节点高级设置中填写真实的 CLI 可执行文件路径。';
const CODEX_WINDOWS_APPS_MESSAGE = `检测到的是 Codex 桌面应用，不是可调用的 Codex CLI。请先运行 ${CODEX_INSTALL_COMMAND}，安装完成后再运行 codex login。`;
const CODEX_NOT_INSTALLED_MESSAGE = `未检测到可执行的 Codex CLI。请先运行 ${CODEX_INSTALL_COMMAND}，安装完成后回到节点点击“刷新”。`;
...
function defaultCodexExecutable() {
  return process.env.JIAREN_CODEX_CLI_PATH || process.env.CODEX_CLI_PATH || 'codex';
}
...
function resolveCodexExecutable(options = {}) {
  ...
  const discoveredCandidates = findCodexCandidates(requested, { env, platform });
  const rejectedWindowsApps = discoveredCandidates.filter(isWindowsAppsPath);
  const candidates = discoveredCandidates.filter((candidate) => !isWindowsAppsPath(candidate));
  const requestedWindowsApps = isWindowsAppsPath(requested);
  ...
  return {
    ...
    fromWindowsApps: requestedWindowsApps || (candidates.length === 0 && rejectedWindowsApps.length > 0),
    candidates,
    rejectedWindowsApps,
...
function spawnCodexProcess(args, options = {}) {
  ...
  if (!resolved.usable || resolved.fromWindowsApps) {
    const error = new Error(resolved.fromWindowsApps ? CODEX_WINDOWS_APPS_MESSAGE : CODEX_NOT_INSTALLED_MESSAGE);
    error.code = resolved.fromWindowsApps ? 'CODEX_DESKTOP_IS_NOT_CLI' : 'CODEX_CLI_NOT_INSTALLED';
    error.codexResolved = resolved;
    throw error;
  }
...
function normalizeCodexAuthStatus(value) {
  const text = String(value || '').trim();
  if (!text) return '';
  if (/logged in using an api key|api[ _-]?key/i.test(text)) return '已通过 API Key 登录';
  if (/logged in using (?:chatgpt|openai)|chatgpt account|openai account/i.test(text)) return '已通过 OpenAI 账号登录';
  return text.replace(/\bsk-[a-z0-9_*.-]+\b/gi, '[已隐藏]');
}
...
function writeCodexLoginCmdScript(invocation) {
  ...
  const content = [
    '@echo off',
    'chcp 65001 > nul',
    'title JiarenAI Codex CLI 登录',
    'echo.',
    'echo JiarenAI Codex CLI 登录流程',
    'echo ------------------------------------------------------------',
    'echo 1. 如果浏览器打开 OpenAI/Codex 授权页，请在浏览器完成登录。',
    'echo 2. 如果终端显示验证码或确认链接，请按终端提示完成授权。',
    'echo 3. 登录完成后回到 JiarenAI 画布，点击 Codex 节点里的“刷新”。',
    'echo.',
    `echo 即将执行: ${escapeCmdEchoText(commandLine)}`,
    'echo.',
    commandLine,
    'set JIAREN_CODEX_LOGIN_EXIT=%ERRORLEVEL%',
    'echo.',
    'if "%JIAREN_CODEX_LOGIN_EXIT%"=="0" (',
    '  echo Codex 登录命令已结束。请回到 JiarenAI 画布点击“刷新”。',
    ') else (',
    '  echo Codex 登录命令返回错误码 %JIAREN_CODEX_LOGIN_EXIT%。',
...
function startCodexLoginInVisibleTerminal(invocation, options = {}) {
  ...
  if (!invocation?.usable || invocation.fromWindowsApps) return null;
  const scriptPath = options.writeLoginScript
    ? options.writeLoginScript(invocation)
    : writeCodexLoginCmdScript(invocation);
  const spawnImpl = options.spawnImpl || spawn;
  const child = spawnImpl('cmd.exe', ['/d', '/k', scriptPath], {
    cwd: config.BASE_DIR,
    windowsHide: false,
    detached: true,
    stdio: 'ignore',
...
function startCodexLogin(options = {}) {
  ...
  if (!invocation.usable || invocation.fromWindowsApps) {
    return {
      started: false,
      executable: invocation.executable,
      command: invocation.commandText,
      installCommand: CODEX_INSTALL_COMMAND,
      code: invocation.fromWindowsApps ? 'codex_desktop_is_not_cli' : 'codex_cli_not_installed',
      message: invocation.fromWindowsApps ? CODEX_WINDOWS_APPS_MESSAGE : CODEX_NOT_INSTALLED_MESSAGE,
    };
  }
  try {
...
function makeCreatorPrompt(body = {}) {
  ...
  instructions.push('你是 JiarenAI 画布中的 Codex 创作者 Agent，优先服务图像、视频、文案、分镜、提示词和创作质检。');
  ...
  const executeAttempt = (attemptArgs) => new Promise((resolve, reject) => {
    ...
    try {
      child = spawnCodexProcess(attemptArgs, {
        ...
        cwd: workspace.dir,
        env: {
          JIAREN_CODEX_WORKSPACE: workspace.dir,
          JIAREN_CODEX_OUTPUT_DIR: workspace.outputDir,
        },
      });
    } catch (error) {
...
async function probeCodexStatus(options = {}) {
  ...
  if (!resolved.usable || resolved.fromWindowsApps) {
    return {
      ...
      resolved,
      installCommand: CODEX_INSTALL_COMMAND,
      message: resolved.fromWindowsApps ? CODEX_WINDOWS_APPS_MESSAGE : CODEX_NOT_INSTALLED_MESSAGE,
    };
  }
  ...
  const loginText = normalizeCodexAuthStatus(login.stdout || login.stderr);
  if (login.code !== 0) {
    return {
      available: false,
...
module.exports = {
  ...
  normalizeCodexAuthStatusForTests: normalizeCodexAuthStatus,
  buildCodexExecArgs,
  ...
  startCodexLoginInVisibleTerminalForTests: startCodexLoginInVisibleTerminal,
  startCodexLogin,
  listCodexSkills,
  createProjectSkill,
  updateProjectSkill,
